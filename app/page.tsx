...
export default function HomePage() {
  ...
  const handleSearch = async (interest: any) => {
    ...
    try {
      ...
      // Log the search parameters and interest object
      console.log('Search parameters:', {
        interest,
        location_input: interest.locations_text || '',
        budget_input: interest.budget_text || '',
        activities_input: interest.activities_text || '',
        notes_input: interest.notes || '',
        match_count: 4
      })

      // Call the search-packages API
      const response = await fetch('/api/suggest-tour', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          location_input: interest.locations_text || '',
          budget_input: `${interest.budget}` || '500', // Convert to number
          accommodation_input: '',
          activities_input: interest.activities_text || '',
          num_participants: `2`,
          preferred_activities: interest.activities_text || '',
          accommodation_preference: 'mid-range',
          budget_range: interest.budget ?
            `${interest.budget}-${parseInt(interest.budget) + 200}` :
            '500-700',
          duration_adjustment: '',
          match_count: "5",
          num_suggestions: "3"
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to search packages: ${errorText}`)
      }

      const data = await response.json()
      console.log('Search API response data:', data)

      // Extract both base tours and suggested tours from response
      const baseTours = Array.isArray(data.packages.base_tours_used) ? data.packages.base_tours_used : []
      const suggestedTours = Array.isArray(data.packages.suggested_tours) ? data.packages.suggested_tours : []

      // Combine both types of tours with a flag to distinguish them
      const packages = [
        ...baseTours.map((tour: Package) => ({ ...tour, isAIGenerated: false })),
        ...suggestedTours.map((tour: Package) => ({ ...tour, isAIGenerated: true }))
      ]

      if (packages.length === 0) {
        console.log('No packages found for the search criteria')
        toast.info('No packages found matching your criteria')
      }
      ...
      toast.error(error instanceof Error ? error.message : "Failed to search packages")
    } finally {
  ...
  useEffect(() => {
    const handleWheel = (e: Event) => {
      ...
      if (wheelEvent.deltaY > 0) {
        ...
        if (sectionHeight > viewportHeight && sectionScrollTop + viewportHeight < sectionHeight + 100) {
          return;
        }
